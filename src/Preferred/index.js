...
export default function Preferred(){
    ...
    useEffect(()=>{
        document.title = "Предпочтения | Yundu"
    },[])
    ...
    const addValue = (href)=>{
        ...
        localStorage.removeItem("subject."+booksSubject)
        }
        else{
        ...
        if(foundIndex > -1){
            ...
            nowState[foundIndex].href = href;
              ...
              delete nowState[foundIndex] 
              nowState = nowState.flat() 
            }
        }
        else{
        ...
        <>
            ...
            <h3>Выбери предмет, а затем предпочитаемый учебник:</h3>
                ...
                {data.filter(obj => obj.heading === localStorage.getItem("class") + " класс").map((obj,index) => {
                    return(
                        <List key={key1} isExact={true} noDelete={true} preferred={preferred} noLink={true} obj={obj} indexVar={index} onClick={selectSubject}/>
                    )
                })}
                </div>
            ...
            : 
            <>
            <button style={{marginTop:0}} onClick={()=>{changePhase(1)}}>Назад</button>
...
return(
    ...
    <label className={styles.radiocontainer}>
        <input type="radio" id={"book" + index} checked={preferred.some(obj1 => obj1.href === obj.href)} value={obj.href} name="book"/>
  <span className={styles.checkmark}></span>
               ...
               <div className={styles.fixed}><button style={{marginRight:10}} onClick={()=>{
                    addValue(null)
                }}>Удалить</button><button onClick={()=>{
                    setBooksRes(null);setBooksSubject(null);changePhase(1)
