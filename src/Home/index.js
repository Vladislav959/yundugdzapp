...
export default function Home(){
    ...
    return(
        <>
            ...
            {preferred.length > 0 ? 
            <> <h3 style={{margin:0}}>Предпочитаемые учебники</h3><div style={{marginBottom:15}} className={styles.menu}>
                {preferred.map((obj,index) => {
                    return(
                        <List isExact={true} isPreferred obj={{heading:"",subjects:[{href:obj.href,subject:obj.name}]}} indexVar={index}/>
                    )
                })}</div>
            </> : null
            }
            <Link to="/preferred"><button style={{marginTop:0}}>Настроить предпочтения</button></Link>
            <h3 style={{margin:0}}>{preferred.length > 0 ? "Остальные предметы" : "Выбери предмет:"}</h3>
